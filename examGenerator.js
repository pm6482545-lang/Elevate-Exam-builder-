...
// --------------------------------------------------------------------
//
// PASS 2 — grade-boundary corrections + difficulty + cover page
// --------------------------------------------------------------------
// 1. GRADE-BOUNDARY BUGS (checked every minGrade tag against the three
//    uploaded curriculum designs line by line — several were wrong):
//      - temperature-kelvin: was 'Grade 9', should be 'Grade 7' (G7 3.6
//        Temperature explicitly includes Celsius<->Kelvin conversion).
//      - inequalities: was 'Grade 8', should be 'Grade 7' (G7 2.3 Linear
//        Inequalities; the G8 design has no inequalities sub-strand).
//      - percentage-error: was 'Grade 8', should be 'Grade 9' (G9 3.6
//        Approximations and Errors; absent from the G8 design).
//      - enlargement-scale-factor: was 'Grade 8', should be 'Grade 9'
//        (G9 4.3 Similarity and Enlargement; G8's 4.3 is Scale Drawing,
//        a different sub-strand — kept separate below).
//      - cuboid-net: was 'Grade 7', should be 'Grade 8' (G8 4.4 Common
//        Solids: sketching nets + surface area from a net; G7's 3.4
//        only covers volume/capacity, never nets).
//      - inverse-proportion-rates (men/hours/days work-rate problems):
//        was 'Grade 8', should be 'Grade 9' (G9 1.4 Compound Proportions
//        and Rates of Work). Added a new, genuinely-G8 topic
//        (direct-indirect-proportion-basic) so Grade 8 papers still get
//        a proportion question, just at the G8 design's actual level.
//    Each fix is called out inline next to its TOPICS entry above.
// 2. DEPTH: added combined-operations-fractions and standard-form-sig-figs
//    (both Grade 8, matching G8 1.2/1.3 exactly), populated the
//    previously-empty statistics-grouped bank (Grade 9, 5.1 Grouped
//    Data), and replaced the single-operation whole-numbers filler MCQ
//    with a genuine order-of-operations (BODMAS) question, per G7 1.1
//    obj. 6 — this was the "1/4 + 2/6"-style shallowness being flagged.
// 3. COVER PAGE: removed "Republic of Kenya" / "Ministry of Education" /
//    the KJSEA/KPSEA standard name entirely — the only exam-name text on
//    the cover is now "ELEVATE KENYA PREDICTIONS". Logo stays top-left
//    and top-right on the cover only.
// 4. WATERMARK: moved from a titlepage-only tikz overlay to a
//    document-wide \AddToShipoutPictureBG hook (eso-pic), so the faint
//    (opacity 0.05) centred logo now renders on the background of EVERY
//    page, not just the cover, while staying too faint to compete with
//    question text.
// --------------------------------------------------------------------
...
const TOPICS = [
    // ---------------- Grade 7 topics ----------------
    {
        ...
        mcq: [
            ...
            (r) => {
                // Order-of-operations (BODMAS) question — G7 1.1 obj. 6, deliberately multi-step rather
                // than a single operation dressed up as a question.
                const a = r.int(20, 90), b = r.int(3, 12), c = r.int(2, 9), d = r.int(4, 30);
                const bodmasAns = a - b * c + d;
                const wrongLeftToRight = ((a - b) * c) + d;
                const distract = [wrongLeftToRight, a - (b * c + d), (a - b) * (c + d)];
                return mcqFromCorrect(r, `Work out $${a} - ${b} \\times ${c} + ${d}$, using the correct order of operations.`, bodmasAns, distract);
            }
        ],
        sectionB: [
            (r, ctx) => {
    ...
    {
        key: 'combined-operations-fractions',
        minGrade: 'Grade 8', // "Carry out combined operations on fractions using correct order of operations" — G8 1.2.
        // Deliberately a separate, harder topic from the Grade 7 fractions-decimals bank above, which only
        // covers single add/subtract of two fractions. This one forces BODMAS across 3 fractions/mixed numbers.
        match: ['combined operation', 'order of operations', 'mixed number'],
        mcq: [
            (r) => {
                const d1 = r.pick([2, 3, 4]), n1 = r.int(1, d1 - 1);
                const d2 = r.pick([3, 4, 5]), n2 = r.int(1, d2 - 1);
                const d3 = r.pick([2, 3, 5]), n3 = r.int(1, d3 - 1);
                // (n1/d1 + n2/d2) x n3/d3
                const sumNum = n1 * d2 + n2 * d1, sumDen = d1 * d2;
                const resNum = sumNum * n3, resDen = sumDen * d3;
                const g = gcd(resNum, resDen);
                const ans = `${resNum / g}/${resDen / g}`;
                const distract = [`${sumNum}/${sumDen}`, `${resNum}/${resDen}`, `${n1}/${d1} + ${n2 * n3}/${d2 * d3}`];
                return mcqFromCorrect(r, `Evaluate $\\left(\\frac{${n1}}{${d1}} + \\frac{${n2}}{${d2}}\\right) \\times \\frac{${n3}}{${d3}}$, giving your answer as a fraction in its simplest form.`, ans, distract);
            }
        ],
        sectionB: [
            (r) => {
                const w1 = r.int(2, 4), n1 = r.int(1, 3), d1 = r.pick([4, 5]);
                const w2 = r.int(1, 3), n2 = r.int(1, 2), d2 = r.pick([2, 3]);
                return {
                    q: `A tailor had $${w1}\\frac{${n1}}{${d1}}$ m of cloth. She used $${w2}\\frac{${n2}}{${d2}}$ m to make a dress and divided what remained equally between two learners for a class project.`,
                    parts: ['Determine the length of cloth each learner received. \\hfill \\textbf{[4 marks]}'],
                    marks: 4
                };
            }
        ]
    ...
    {
        key: 'standard-form-sig-figs',
        minGrade: 'Grade 8', // Standard form + significant figures — G8 1.3 Decimals.
        match: ['standard form', 'significant figure', 'recurring decimal'],
        mcq: [
            (r) => {
                const digits = r.int(2, 9), decimals = r.int(10, 99);
                const wholePart = `${digits}${decimals}`;
                const magnitude = wholePart.length - 1;
                const numeral = parseFloat(`${digits}.${decimals}`);
                const value = numeral * Math.pow(10, magnitude);
                const ans = `${numeral} \\times 10^{${magnitude}}`;
                const distract = [`${numeral} \\times 10^{${magnitude - 1}}`, `${numeral} \\times 10^{${magnitude + 1}}`, `${digits}.${decimals} \\times 10^{${magnitude + 2}}`];
                return mcqFromCorrect(r, `Express ${Math.round(value).toLocaleString()} in standard form.`, ans, distract);
            }
        ],
        sectionB: [
            (r) => {
                const val = (r.int(314, 987) / 100).toFixed(2);
                const sf = r.pick([1, 2, 3]);
                return {
                    q: `A learner measured the length of a desk as ${val} m.`,
                    parts: [`Express ${val} m correct to ${sf} significant figure${sf > 1 ? 's' : ''}. \\hfill \\textbf{[2 marks]}`],
                    marks: 2
                };
            }
        ]
    ...
    {
        // MERGED: temperature-kelvin used to be a separate topic gated to 'Grade 9', which was wrong
        // (Kelvin<->Celsius is a Grade 7 objective, 3.6 obj. 4) AND redundant with this topic once fixed —
        // both would sit at minGrade 'Grade 7' and the earlier-declared 'temperature' entry would always
        // win the match (findTopic used first-match-wins), so the Kelvin question could never actually
        // fire. Folding it in here as a second MCQ template removes that shadowing bug entirely.
        key: 'temperature',
        minGrade: 'Grade 7',
        match: ['temperature', 'celsius', 'thermometer', 'kelvin', 'absolute temperature'],
        mcq: [
            (r) => {
    ...
    {
        key: 'cuboid-net',
        minGrade: 'Grade 8', // FIX: sketching nets and calculating surface area from a net is Grade 8
        // sub-strand 4.4 "Common Solids". Grade 7's 3.4 only covers volume/capacity of cubes, cuboids and
        // cylinders — it never mentions nets — so this was leaking Grade-8-only content into G7 papers.
        match: ['net of a', 'nets of solids', 'net of solid', 'sketch nets', 'nets of cubes', 'common solids', 'surface area from the net', 'surface area from a net'],
        mcq: [],
        sectionB: [
            (r) => {
    ...
    },
    {
        ...
        minGrade: 'Grade 9', // FIX: the men/hours/days combined work-rate problem (person-days style) is
        // Grade 9 sub-strand 1.4 "Compound Proportions and Rates of Work". Grade 8's 1.5 covers only basic
        // direct/indirect proportion, not multi-variable rate-of-work problems — this was G9-only content
        // leaking into G8 papers.
        match: ['inverse proportion', 'direct proportion', 'person-days', 'work rate', 'rates of work', 'compound proportion'],
        mcq: [],
        sectionB: [
            (r) => {
    ...
    {
        key: 'direct-indirect-proportion-basic',
        minGrade: 'Grade 8', // Direct & indirect proportion — G8 1.5 Rates, Ratio, Proportion and Percentages.
        // Kept separate from inverse-proportion-rates (Grade 9) so Grade 8 papers still get a proportion
        // question, but at the two-variable level the G8 design actually specifies.
        match: ['direct proportion', 'indirect proportion', 'proportional'],
        mcq: [
            (r) => {
                const workers1 = r.pick([4, 5, 6, 8]), days1 = r.pick([6, 9, 10, 12]);
                const workers2 = r.pick([workers1 * 2, workers1 + 2, Math.max(2, workers1 - 2)].filter(w => w !== workers1));
                const days2 = (workers1 * days1) / workers2;
                const clean = Number.isInteger(days2);
                const ans = clean ? days2 : days2.toFixed(1);
                const distract = [days1, (days1 * workers2 / workers1).toFixed(1), (ans === days1 ? ans + 1 : ans)];
                return mcqFromCorrect(r, `It takes ${workers1} workers ${days1} days to dig a trench, all working at the same rate. How many days would it take ${workers2} workers, working at the same rate, to dig the same trench?`, ans, distract);
            }
        ],
        sectionB: [
            (r) => {
                const litres1 = r.pick([8, 12, 15, 20]), cost1 = r.pick([320, 480, 600, 750]);
                const litres2 = r.pick([litres1 + 5, litres1 * 2, litres1 - 3].filter(l => l > 0 && l !== litres1));
                return {
                    q: `${r.pick(['Njeri', 'Barasa', 'Chebet'])} paid Ksh ${cost1} for ${litres1} litres of petrol.`,
                    parts: [`If petrol cost is directly proportional to the number of litres bought, determine how much ${r.pick(['she', 'he'])} would pay for ${litres2} litres. \\hfill \\textbf{[3 marks]}`],
                    marks: 3
                };
            }
        ]
    },
    {
        ...
        minGrade: 'Grade 9', // FIX: "Approximations and Errors" (percentage error specifically) is Grade 9
        // sub-strand 3.6 — it does not appear anywhere in the Grade 8 curriculum design.
    ...
    {
        key: 'inequalities',
        minGrade: 'Grade 7', // FIX: simple + compound linear inequalities in one unknown, represented on a
        // number line, are a Grade 7 objective (2.3 Linear Inequalities). The Grade 8 curriculum design has
        // no inequalities sub-strand at all — Grade 9 revisits them only for graphical/two-unknown work.
    ...
    },
    {
        ...
        minGrade: 'Grade 8',
        // Longer, more specific phrases listed first so the specificity-based matching in
        // findTopic() reliably beats the generic 'linear equation' keyword on the algebra topic.
        match: ['linear equations in two unknowns', 'equations in two unknowns', 'simultaneous', 'two unknowns', 'elimination'],
        mcq: [],
        sectionB: [
            (r) => {
    ...
    },
    {
        ...
        minGrade: 'Grade 9', // FIX: enlargement / linear scale factor belongs to Grade 9 sub-strand 4.3
        // "Similarity and Enlargement". Grade 8's 4.3 is Scale Drawing (a different sub-strand — see
        // scale-drawing below), not enlargement, so this was leaking Grade-9-only content into G8 papers.
    ...
    },
    {
        ...
        minGrade: 'Grade 9', // Grouped data: class width, modal class, mean/median of grouped data — G9 5.1
        match: ['grouped data', 'class width', 'frequency distribution', 'modal class'],
        mcq: [
            (r) => {
                const lower = r.pick([0, 10, 20]), width = r.pick([5, 10]);
                const upper = lower + width - 1;
                const distract = [width, width - 1, width + 1];
                return { q: `A class in a grouped frequency distribution is $${lower} - ${upper}$. What is the class width?`,
                    a: `${width}`, b: `${width - 1}`, c: `${width + 1}`, d: `${upper}` };
            }
        ],
        sectionB: [
            (r) => {
                const classes = [[10, 19], [20, 29], [30, 39], [40, 49], [50, 59]];
                const freqs = classes.map(() => r.int(2, 14));
                const table = `\\begin{center}\\begin{tabular}{|c|c|}\\hline\nMarks & Frequency \\\\\\hline\n${classes.map((c, i) => `${c[0]} - ${c[1]} & ${freqs[i]}`).join(' \\\\\\hline\n')} \\\\\\hline\n\\end{tabular}\\end{center}`;
                return {
                    q: `The table below shows marks scored by learners in a Mathematics test.\\\\ ${table}`,
                    parts: [
                        '(a) State the modal class. \\hfill \\textbf{[1 mark]}',
                        '(b) Using an assumed mean, calculate the mean mark. \\hfill \\textbf{[4 marks]}'
                    ],
                    marks: 5
                };
            }
 ...
 *
 * FIX: this used to be TOPICS.find(...) — first-match-wins in array order. That let a
 * generic keyword on an EARLIER topic (e.g. 'temperature', 'equation', 'cuboid') silently
 * shadow a more specific, correctly-scoped topic declared LATER (e.g. a Kelvin-specific
 * question, simultaneous-equations, cuboid-net) even when both were grade-eligible — the
 * more specific topic's questions would just never fire. Instead, score every eligible
 * topic by the length of its longest matching keyword (a proxy for specificity — 'net of a'
 * beats 'cuboid') and take the best one, falling back to array order only on an exact tie.
 */
function findTopic(row, ctx) {
    ...
    for (const t of TOPICS) {
        if (allowed && !allowed.includes(t.minGrade)) continue;
        let longestMatch = -1;
        for (const kw of t.match) {
            if (hay.includes(kw) && kw.length > longestMatch) longestMatch = kw.length;
        }
        if (longestMatch > bestScore) { bestScore = longestMatch; best = t; }
    }
    return best;
}
...
function buildLatex(ctx, mcqs, sectionB) {
    ...
    tex += `\\usepackage{amsmath,amssymb,tikz,graphicx,multicol,eso-pic}\n`;
    tex += `\\usepackage{geometry}\n`;
    tex += `\\geometry{top=15mm, bottom=15mm, left=15mm, right=15mm}\n\n`;

    // Watermark: applied to the BACKGROUND OF EVERY PAGE (not just the cover) via eso-pic's
    // \AddToShipoutPictureBG hook, which re-runs once per page automatically. Kept at very low
    // opacity (0.05) so it never competes with question text, on the cover or on content pages.
    ...
    // Cover header: logo top-left and top-right only; the only exam-name text is
    // "ELEVATE KENYA PREDICTIONS" -- no Republic of Kenya / Ministry of Education / KJSEA wording.
    tex += `\\vspace*{-5mm}\n`;
    ...
    tex += `{\\Large \\textbf{ELEVATE KENYA PREDICTIONS}} \\\\[0.3em]\n`;
    tex += `{\\normalsize \\textbf{${ctx.gradeClean.toUpperCase()}}}\n\\end{minipage}\n`;
    tex += `\\begin{minipage}{0.16\\textwidth}\\centering\\includegraphics[width=2cm]{logo.png}\\end{minipage}\n`;
